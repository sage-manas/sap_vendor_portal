...
- `PurchaseOrderItem[]` (`poPk` FK, cascade delete): `line, materialCode, description, quantity, grnQuantity, unitPrice, netValue, uom, plant` (plant lives here, not on the header — issue #62: a multi-line order can ship from more than one plant), and a 1:1 optional `InvoicePlan`.
...
**Ad-hoc scripts:** none. `backend/` used to carry a handful of hand-run probe/`test_*` scripts (one of which printed every client and vendor, emails included); they were deleted and `*.tmp.js` is gitignored. Real tooling lives in `backend/scripts/`.
