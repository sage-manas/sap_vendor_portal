...
Every read this portal performs against your SAP system, and the one write that creates a
vendor master record, go through eleven custom Z REST services rather than the standard SAP
OData APIs. `backend/sap/drivers/s4odata.driver.js` — the code that calls them — documents
...
## The eleven endpoints, and what each exposes
...
| `/zvendor_create/VENDOR_CR` | POST | **Write.** Creates a vendor master record from a flat JSON payload — company name, GSTIN, PAN, bank details, address. The one write in this list; an unauthenticated version of this endpoint lets anyone who can reach it create vendor master records in your system. |
...
The first eight carry real business and financial data — vendor master records, full purchase
history, invoice amounts, and bank transfer references among them. The last three are
reference/catalogue data and lower risk, but are included for a complete accounting of every
endpoint this integration touches.

## What we are asking for

1. **Authentication enabled on all eleven**, with a technical user (a service account, not a
...
authentication for the eleven that exist today, and answering the filtering questions
